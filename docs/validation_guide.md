...
The current uncalibrated baseline is NSE `0.9076`, RMSE `1417.1 m³/min`,
percent bias `+3.69%`, and Pearson `r = 0.9854`. The tracked results JSON is
regenerated by the command and guarded by `tests/test_real_validation_case.py`.
These scores establish a baseline for subsequent geometry and numerical changes;
they do not make the estimated width, slope, or roughness calibrated.
