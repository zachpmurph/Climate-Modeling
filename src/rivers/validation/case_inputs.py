...
def _disabled_legacy_1d_validation_case(
    config_path, *, output_path=None, overrides=None
):
    """Run one configured Saint-Venant case and return its validation evidence."""
    raise RuntimeError(
        "Observed 1-D validation is disabled. Use the canonical 2-D "
        "run_validation_case entry point."
    )
    ...
    evidence = {
        ...
        "error_diagnosis": diagnose_hydrograph(
            target_times, target_flow, scores["predicted_on_obs"]
        ),
        "reach_diagnosis": diagnose_reach_routing(
            target_times,
            upstream_on_target,
            target_flow,
            scores["predicted_on_obs"],
        ),
        "mass": {
...
def run_validation_case(config_path, *, output_path=None, overrides=None):
    """Run the canonical observed validation through Saint-Venant 2-D only."""
    if overrides:
        raise ValueError(
            "Generic case overrides are disabled in canonical validation. "
            "Use explicit 2-D structural variants in run_sensitivity.py."
        )
    config_path = Path(config_path)
    config = json.loads(config_path.read_text(encoding="utf-8"))
