...
| **4ag — Geometry-aware observed-event startup** | Current branch | Extended the two-gauge validation harness to rectangular, trapezoidal, compound stage–width, and raw surveyed asymmetric sections. Initial depth is now solved per cell from local geometry, slope, roughness, and initial flow before observed spin-up. | Solver support for changing depth and width is not useful for NSE/correlation work if validation silently converts every reach back to one rectangular width. Geometry-consistent startup prevents a false transient from being scored as routing error. |
